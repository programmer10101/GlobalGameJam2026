...
export class GameScene3 implements Scene {
  /* ---------- Layers ---------- */
  ...
  private computeLayout() {
    ...
    const scale = Math.min(
      rect.width / this.docImage.width,
      rect.height / this.docImage.height
    )
...
private drawDocument() {
  ...
  const roundText = `Round Score: ${this.roundScore}`
  const totalText = `Total Score: ${this.totalScore}`
  ...
  this.exitButton = this.drawButton("EXIT", w / 2, h * 0.65)
}
  ...
  // Hand control back to MenuScene
  // Assumes your SceneManager exposes this pattern
  (window as any).game?.sceneManager?.setMenu()
}
  ...
  private calculateScore(): number {
    ...
    for (const r of targetRects) {
      for (
        let y = Math.floor(r.y * h);
        y < Math.ceil((r.y + r.height) * h);
        y += step
      ) {
        for (
          let x = Math.floor(r.x * w);
          x < Math.ceil((r.x + r.width) * w);
          x += step
        ) {
          const i = (y * w + x) * 4
