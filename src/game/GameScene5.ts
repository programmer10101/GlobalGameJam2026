...
export class GameScene5 implements Scene {
  ...
  private calculateScore(): number {
    ...
    const step = 4
    let correct = 0, missed = 0, falsePositive = 0

    for (let y = 0; y < h; y += step) {
      for (let x = 0; x < w; x += step) {
        const i = (y * w + x) * 4
        if (data[i + 3] === 0) continue
        const inside = targetRects.some(r => x >= r.x * w && y >= r.y * h && x <= (r.x + r.width) * w && y <= (r.y + r.height) * h)
